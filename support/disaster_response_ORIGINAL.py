...
''')
	...
	for threshold in thresholds:
		...
		if metric == 'hamming_loss':
			...
			score = metrics_dct[metric](y_true, pred, average='macro')
			if score > top_score:
				replace = True
...
def plot_history(history,
	...
	if len(history) == 2 * len(plots):
		val = True
	plot1 = [x for x in history.keys() 
			 if 'loss' in x and 'val' not in x][0]
	plot2 = [x for x in history.keys() 
			 if 'recall' in x and 'val' not in x][0]
